...
(function (global) {
  ...
  async function fetchMockQuestionsFromAI(exam, count) {
    count = count || 40;
    const cacheKey = 'mock_fs_' + exam + '_' + new Date().toDateString().replace(/ /g,'_');
    const cached = lsGet(cacheKey, 'null');
    if (cached && Array.isArray(cached) && cached.length >= Math.min(count, 20)) return cached.slice(0, count);
    try {
const qs = await _loadQuestionsFromStorage(
  ...
  const Referral = {
    ...
    registerReferral() {
      ...
      if (n >= this.REFS_NEEDED) {
        ...
        toast('🎉 3 referrals complete! Mock Test unlocked! 🏆', 4000);
...
async function _generateQuizQuestions(exam, count, type) {
  count = count || 10;
  try {
    // Load from Firebase Storage based on type
    const folder = type === 'pyq' ? 'pyq' : 'mock';
    const qs = await _loadQuestionsFromStorage(folder + '/' + exam + '/questions.json', count);
    if (qs && qs.length) return _shuffleArray(qs).slice(0, count);
    // Fallback to generic battle questions
    const qs2 = await _loadQuestionsFromStorage('battles/' + exam + '/questions.json', count);
    if (qs2 && qs2.length) return _shuffleArray(qs2).slice(0, count);
    return [];
  } catch(e) { return []; }
}
    ...
    _renderExamExpansion() {
      ...
      body.innerHTML = `
        ...
        <div class="cf-exam-grid">
          ${exams.map(([k,v])=>`
            <button class="cf-exam-chip" style="--ec:${v.color}" onclick="CF.closeModal('cf-exam-modal');CF._pyqState.exam='${k}';CF.openMockTest()">
              ${v.label}
            </button>`).join('')}
        </div>
    ...
    _renderDailyGoal() {
      ...
      body.innerHTML = `
        ...
        <div style="display:flex;gap:8px;margin-top:20px;flex-wrap:wrap">
          <button class="cf-btn cf-btn-primary" onclick="CF.closeModal('cf-daily-modal');CF.openMockTest()">🎯 Take Mock Test</button>

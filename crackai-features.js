...
async function _generateQuizQuestions(exam, count, type) {
  ...
  // Gate battle access before joining
  const _origOpenGroupChat = CF._openGroupChat;
  CF._openGroupChat = async function(groupId) {
    const access = await CF._checkBattleAccess(groupId);
    if (!access.allowed) {
      if (typeof toast === 'function') toast('🔒 ' + access.reason);
      if (typeof openPremiumModal === 'function') openPremiumModal();
      return false;
    }
    // Track the battle join for free users
    CF._trackBattleJoin(groupId);
    return await _origOpenGroupChat.call(this, groupId);
  };
  ...
  CF._trackBattleJoin = function(groupId) {
    ...
    if (!isPremium) {
      const today = new Date().toISOString().split('T')[0];
      const key = 'sscai_battles_' + today;

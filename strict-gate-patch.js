...
(function () {
  ...
  //
  // NOTE: server-side /api/status re-verification (payment-security-patch.js)
  // was disabled to cut Cloud Functions invocations + Firestore reads from
  // the 60s polling loop. The /api/deepseek endpoint still independently
  // enforces premium + daily limits server-side, so this is a UI-trust-only
  // gate — actual AI usage cannot be spoofed via localStorage.
  function isPremium() {
    try {
  ...
  window.handleLimitHit = function (type) {
    var labels = { text: 'Daily text limit reached (3/day)', image: 'Daily image limit reached', pdf: 'Daily PDF limit reached' };
  ...
  console.info('[StrictGate] v2.2 — companion removed, 3 free messages, new plans active');

})();
/* ── PYQ Bank & Mock Test — fix premium check for paid users ── */
(function patchCFPYQ() {
  ...
  function patch() {
    ...
    window.CF.openPYQ = function () {
      if (!robustIsPremium()) {
        try { if (typeof showToast === 'function') showToast('🔒 PYQ Bank requires Premium or 3 referrals \u20b9199/mo'); } catch(e){}
        try { if (typeof openPremiumModal === 'function') openPremiumModal(); } catch(e){}
        return;
      }
      window.CF.openModal('cf-pyq-modal');
      window.CF._renderPYQHome();
    };
    window.CF.openMockTest = function () {
      if (!robustIsPremium()) {
        try { if (typeof showToast === 'function') showToast('🔒 Mock Test requires Premium or 3 referrals \u20b9199/mo'); } catch(e){}
        try { if (typeof openPremiumModal === 'function') openPremiumModal(); } catch(e){}
        return;
      }
    ...
    console.info('[StrictGate] CF.openPYQ + CF.openMockTest patched with robust premium check');
  }
  patch();
})();

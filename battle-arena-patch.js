/**
 * battle-arena-patch.js — CrackwithAI Online Battle Arena v3.1
 * ═══════════════════════════════════════════════════════════════════
 *  FEATURES:
...
(function (global) {
  ...
  async function _getDemoQuestionsForExam(examKey) {
    ...
    // 2. Check Firebase Firestore cache (stores AI-generated questions)
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          if (data.questions && data.questions.length >= 10) {
            // Normalize IDs and use seen tracker for this fallback pool too
            const pool2 = data.questions.map((q, i) => ({ ...q, id: q.id || q.q || ('demo_' + examKey + '_' + i) }));
            const picked2 = _battleSeenTracker.pick('demo_' + examKey, pool2, 10);
            if (picked2 && picked2.length >= 10) return picked2;
            // If tracker exhausted, return a fresh shuffle of the pool
    ...
    // 4. Generate via AI and cache for future use
    const uniqueSeed = 'demo_' + examKey + '_' + Date.now() + '_' + Math.random().toString(36).slice(2,6);
    const questions = await _generateBattleQuestionsUnique(examKey, 10, uniqueSeed);
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      .ba-card-name { font-size: 15px; font-weight: 800; color: #fff; letter-spacing: -0.01em; line-height: 1.3; }
  ...
  window.BA = {
          ...
          <div class="ba-gate-box">
            ...
            <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">
              <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.3);border-radius:10px;padding:10px 12px;">
                <div><div style="font-size:13px;font-weight:700;color:#fff;">⚔️ Basic</div><div style="font-size:11px;color:rgba(200,195,255,0.5);">5 battles/month</div></div>
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(239,68,68,0.12);border:1.5px solid rgba(239,68,68,0.5);border-radius:10px;padding:10px 12px;position:relative;">
                ...
                <div><div style="font-size:13px;font-weight:700;color:#fff;">⚔️⚔️ Pro</div><div style="font-size:11px;color:rgba(200,195,255,0.5);">19 battles/month</div></div>
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(245,158,11,0.07);border:1px solid rgba(245,158,11,0.35);border-radius:10px;padding:10px 12px;">
                <div><div style="font-size:13px;font-weight:700;color:#fff;">⚔️🏆 Academy</div><div style="font-size:11px;color:rgba(200,195,255,0.5);">29 battles/month</div></div>
    ...
    async _joinDemoBattle(demoId) {
      ...
      const _demoResults = () => {
        ...
        // Award coins in demo battle (same as real battle)
        const myDemoRank = sorted.findIndex(([u]) => u === myUid);
        let demoCoinWon = 0;
        if (sorted.length >= 10) {
          if (myDemoRank === 0) demoCoinWon = 50;
          else if (myDemoRank === 1) demoCoinWon = 30;
          else if (myDemoRank === 2) demoCoinWon = 15;
        ...
        const demoAwardKey = 'ba_coins_demo_' + demoId + '_' + myUid;
        if (demoCoinWon > 0 && !localStorage.getItem(demoAwardKey)) {
          localStorage.setItem(demoAwardKey, '1');
          const _demoAward = () => {
            if (typeof window.addCoins === 'function') {
              window.addCoins(demoCoinWon, 'Demo battle win 🎯');
            } else {
              try {
                ...
                cur.coins = (cur.coins || 0) + demoCoinWon;
                localStorage.setItem(k, JSON.stringify(cur));
                if (typeof showToast === 'function') showToast(`🪙 +${demoCoinWon} coins! (Demo battle win 🎯)`, 2500);
              } catch(_) {}
            }
          };
          if (typeof window.addCoins === 'function') { _demoAward(); }
          else { setTimeout(_demoAward, 300); }
        }

        body.innerHTML = `
          ${demoCoinWon > 0 ? `<div style="text-align:center;font-size:18px;font-weight:800;color:#f59e0b;margin-bottom:8px;">🪙 +${demoCoinWon} Coins Earned!</div>` : ''}
    ...
    _showCreateForm() {
              ...
              <div style="background:rgba(108,99,255,0.08);border:1px solid rgba(108,99,255,0.3);border-radius:12px;padding:12px;margin-bottom:10px;text-align:left;">
                ...
                <div style="display:flex;flex-direction:column;gap:6px;">
                  <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,0.04);border-radius:8px;padding:8px 10px;">
                    <span style="font-size:12px;color:#fff;">⚔️⚔️ Pro — 19 battles/mo</span>
                  ...
                  <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(245,158,11,0.06);border-radius:8px;padding:8px 10px;">
                    <span style="font-size:12px;color:#fff;">⚔️🏆 Academy — 29 battles/mo</span>
      ...
      body.innerHTML = `
        ...
        <div style="margin-bottom:12px;">
              <label style="font-size:11px;color:rgba(200,195,255,0.55);font-weight:600;display:block;margin-bottom:6px;">📝 Number of Questions</label>
              <select id="ba-question-count" style="width:100%;padding:10px 12px;border-radius:12px;border:1.5px solid rgba(108,99,255,0.3);background:rgba(255,255,255,0.05);color:#fff;font-size:13px;font-family:'Plus Jakarta Sans',sans-serif;cursor:pointer;outline:none;appearance:none;-webkit-appearance:none;">
    ...
    async _renderLeaderboard() {
      ...
      try {
        ...
        // Fetch weekly top 100 (more than 50 to ensure we get all real players)
        let weeklyEntries = [];
        try {
          const q = query(collection(db, 'battleLeaderboard'), where('weekKey','==',weekKey), orderBy('xp','desc'), limit(100));
          const snap = await getDocs(q);
          ...
          try {
            ...
            weeklyEntries = snap2.docs.map(d => d.data()).sort((a,b)=>(b.xp||0)-(a.xp||0)).slice(0,100);
          } catch(e2) {
        ...
        // Merge my local XP if not in Firestore yet
        ...
        // Sort all real users to the top
        weeklyEntries.sort((a,b) => (b.xp||0) - (a.xp||0));

        // Add demo players only if list is very sparse (< 5 real players)
        if (weeklyEntries.length < 5) {
          const _demoPlayers = [
            { uid:'_demo_1', name:'Arjun Sharma', xp:340, battles:18, wins:12, weekKey, _demo:true, avatar:'🔥' },
            { uid:'_demo_2', name:'Priya Verma', xp:290, battles:15, wins:9, weekKey, _demo:true, avatar:'👑' },
            { uid:'_demo_3', name:'Rahul Singh', xp:260, battles:14, wins:8, weekKey, _demo:true, avatar:'⚡' },
            { uid:'_demo_4', name:'Sneha Patel', xp:230, battles:12, wins:7, weekKey, _demo:true, avatar:'🌟' },
            { uid:'_demo_5', name:'Vikram Nair', xp:200, battles:11, wins:6, weekKey, _demo:true, avatar:'🧠' },
            { uid:'_demo_6', name:'Anjali Gupta', xp:170, battles:9, wins:5, weekKey, _demo:true, avatar:'🛡️' },
            { uid:'_demo_7', name:'Rohan Mehta', xp:150, battles:8, wins:4, weekKey, _demo:true, avatar:'🚀' },
            { uid:'_demo_8', name:'Kavya Reddy', xp:130, battles:7, wins:3, weekKey, _demo:true, avatar:'💎' },
            { uid:'_demo_9', name:'Amit Yadav', xp:110, battles:6, wins:2, weekKey, _demo:true, avatar:'🥷' },
            { uid:'_demo_10', name:'Divya Joshi', xp:90, battles:5, wins:1, weekKey, _demo:true, avatar:'🐉' },
          ];
          // Only add demo players whose UID isn't already in the list
          _demoPlayers.forEach(dp => {
            if (!weeklyEntries.find(e => e.uid === dp.uid)) weeklyEntries.push(dp);
          });
        }

        // Final sort by XP (real users + demo users)
        weeklyEntries.sort((a,b) => (b.xp||0) - (a.xp||0));
    ...
    async _renderAllTimeLeaderboard() {
      ...
      try {
        const db = window._firebaseDb;
        const { collection, query, orderBy, limit, getDocs } = window._firebaseFns;
        const myUid = uid();
        let entries = [];
        try {
          const q = query(collection(db, 'battleLeaderboardAllTime'), orderBy('xp', 'desc'), limit(100));
          const snap = await getDocs(q);
          entries = snap.docs.map(d => d.data());
        } catch(e) {
          // Fallback without index
          const snap2 = await getDocs(collection(db, 'battleLeaderboardAllTime'));
          entries = snap2.docs.map(d => d.data()).sort((a, b) => (b.xp || 0) - (a.xp || 0)).slice(0, 100);
        }

        // Add demo players for social proof only if list is very sparse (< 5 real players)
        if (entries.length < 5) {
          const _allTimeDemoPlayers = [
            { uid:'_demo_all_1', name:'Arjun Sharma', xp:1240, battles:58, wins:42, _demo:true, avatar:'🔥' },
            { uid:'_demo_all_2', name:'Priya Verma', xp:1090, battles:52, wins:38, _demo:true, avatar:'👑' },
            { uid:'_demo_all_3', name:'Rahul Singh', xp:980, battles:48, wins:35, _demo:true, avatar:'⚡' },
            { uid:'_demo_all_4', name:'Sneha Patel', xp:890, battles:44, wins:31, _demo:true, avatar:'🌟' },
            { uid:'_demo_all_5', name:'Vikram Nair', xp:750, battles:38, wins:26, _demo:true, avatar:'🧠' },
            { uid:'_demo_all_6', name:'Anjali Gupta', xp:680, battles:34, wins:23, _demo:true, avatar:'🛡️' },
            { uid:'_demo_all_7', name:'Rohan Mehta', xp:590, battles:29, wins:19, _demo:true, avatar:'🚀' },
            { uid:'_demo_all_8', name:'Kavya Reddy', xp:520, battles:26, wins:16, _demo:true, avatar:'💎' },
            { uid:'_demo_all_9', name:'Amit Yadav', xp:450, battles:22, wins:13, _demo:true, avatar:'🥷' },
            { uid:'_demo_all_10', name:'Divya Joshi', xp:380, battles:19, wins:10, _demo:true, avatar:'🐉' },
          ];
          // Only add demo players whose UID isn't already in the list
          _allTimeDemoPlayers.forEach(dp => {
            if (!entries.find(e => e.uid === dp.uid)) entries.push(dp);
          });
        }
  ...
  async function _loadBattlePool(exam) {
    ...
    if (!storage || !sRef || !getDLUrl) return null;
    const paths = [
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data) && data.length) {
            const pool = data.map((q, i) => ({ ...q, id: q.id || q.q || ('b_' + exam + '_' + i) }));
  ...
  async function _generateBattleQuestions(exam, count, uniqueSeedParam) {
    ...
    return Array.from({length: count}, (_, i) => ({
      q: 'Q' + (i+1) + ': Upload questions to Firebase Storage at battles/' + exam + '/questions.json',
      opts: ['Option A', 'Option B', 'Option C', 'Option D'],
      ans: 0, topic: 'General', exp: 'Upload question bank to gs://rankgpt-f8a64.firebasestorage.app/battles/' + exam + '/questions.json'
    }));
  }

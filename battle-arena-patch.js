...
(function (global) {
  ...
  async function _getDemoQuestionsForExam(examKey) {
    ...
    if (window._firebaseDb && window._firebaseFns) {
      try {
    // Normalize question format to ensure q, opts, ans fields
    try {
      if (Array.isArray(pool) && pool.length) {
        const normalized = pool.map(q => ({
          ...
          id: q.id || q.q || ("q_" + Math.random())
        })).filter(q => q.q && q.opts.length === 4);
        if (normalized.length >= 10) return normalized.slice(0, Math.min(10, normalized.length));
      }
    } catch(e) {}
  ...
  window.BA = {
      ...
      try {
        ...
        if (data.status === 'generating') {
          ...
          if (!data.questions || data.questions.length === 0) {
            // Questions might not be in first active fetch, retry in 200ms
    ...
    _renderActiveQuiz(battle) {
      ...
      if (!questions || !Array.isArray(questions) || questions.length === 0) {
        // Show loading screen if questions not ready yet
        body.innerHTML = `
          <div style="text-align:center;padding:48px 16px;">
            <div class="ba-spinner" style="width:48px;height:48px;border-width:4px;margin:0 auto 20px;"></div>
  ...
  async function _generateBattleQuestions(exam, count, uniqueSeedParam) {
    ...
    return Array.from({length: count}, (_, i) => ({
      ...
      ans: 0, topic: 'General', exp: 'Upload question bank to gs://rankgpt-f8a64.firebasestorage.app/mock/' + exam + '/questions.json'
    }));
  }

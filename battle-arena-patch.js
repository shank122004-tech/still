...
(function (global) {
  ...
  window.BA = {
    ...
    _stopPolling() {
      ...
      // Reset render hash and countdown flag when stopping polling
      ...
      if (!document.getElementById('ba-gen-elapsed')) {
        ...
        body.innerHTML = `
          <div style="text-align:center;padding:48px 16px;">
            ...
            <div style="font-size:20px;font-weight:800;color:#fff;margin-bottom:8px;">⏳ Loading questions from question bank...</div>
    ...
    _renderBattleRoom(battle) {
      ...
      // Only count players who are ready/inside the battle room
      const allPlayers = (battle.players || []);
      const readyPlayers = (battle.readyPlayers || {});
      
      // Only count players who are ready/inside the battle room
      const readyPlayersList = allPlayers.filter(uid => readyPlayers[uid] === true);
      const playerCount = readyPlayersList.length;
      
      // Get names only for ready players
      const playerNames = readyPlayersList.map(uid => battle.playerNames ? battle.playerNames[uid] : 'Player');
      ...
      if (battle.status === 'waiting') {
        ...
        body.innerHTML = `
          ...
          <div style="background:rgba(74,222,128,0.08);border:1px solid rgba(74,222,128,0.2);border-radius:12px;padding:14px;margin:12px 0;text-align:center;">
            ...
            <div style="font-size:12px;color:rgba(200,195,255,0.5);margin-top:4px;">${playerCount}/${MAX_PLAYERS} ready · ${MAX_PLAYERS - playerCount} waiting</div>
          </div>
          <div class="ba-section-title">👥 Players Joined</div>
    ...
    _renderActiveQuiz(battle) {
      ...
      if (!questions || !Array.isArray(questions) || questions.length === 0) {
        body.innerHTML = `
          <div style="text-align:center;padding:48px 16px;">
            <div class="ba-spinner" style="width:48px;height:48px;border-width:4px;margin:0 auto 20px;"></div>
            <div style="font-size:18px;font-weight:800;color:#fff;margin-bottom:8px;">⏳ Loading questions...</div>
            <div style="font-size:13px;color:rgba(200,195,255,0.55);">Your battle is starting now</div>
          </div>`;
      ...
      // This prevents timer flicker and keeps options clickable during polling
      const renderHash = qi + '|' + (answered ? 'answered' : 'open') + '|' + JSON.stringify(quiz.xp || {});
      if (this._lastRenderHash === renderHash && document.getElementById('ba-qtimer-fill')) {
    ...
    _startQuestionTimer(battleId, qi, questionStartedAt) {
      ...
      // FIX: Use 100ms interval for more accurate timing updates
      this._questionTimer = setInterval(tick, 100);
    },
          ...
          <div style="background:linear-gradient(135deg,rgba(108,99,255,0.14),rgba(108,99,255,0.06));border:1.5px solid rgba(108,99,255,0.35);border-radius:16px;padding:16px 18px;margin-bottom:18px;box-shadow:0 8px 28px rgba(108,99,255,0.12);">
            <div style="display:flex;align-items:center;gap:14px;margin-bottom:12px;">
              ...
              <div style="flex:1;">
                ...
                <div style="font-size:12px;color:rgba(200,195,255,0.6);margin-top:2px;">Level ${calculateLevel(myData.xp || 0)} ${levelData.title}</div>
              </div>
            </div>
